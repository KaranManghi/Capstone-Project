...
class lidar:
    def __init__(self,detection_range,channels,dust_resistance,noise,security,accuracy,battery_consumption,isSun):
        ...
        self.default_security_weightage=0.05
        self.default_accuracy_weightage=0.3
        self.default_battery_consumption_weightage=0.1

class camera:
    def __init__(self,megapixels,ois,autofocus,depth_sensor,hdr,flash,aperture,battery_consumption,security):
        self.type="camera"
        self.score=None
        ...
        self.battery_consumption=battery_consumption #generated randomly
        #more the noise value higher is the noise score. Inverse of noise level
        self.security=security

        self.default_accuracy_weightage=0.7
        self.default_megapixels_weightage_for_accuracy=0.2
        self.default_aperture_weightage_for_accuracy=0.1
        
        
        # self.default_noise_weightage=0.3
        self.default_ois_weightage_for_accuracy=0.2
        self.default_autofocus_weightage_for_accuracy=0.2
        self.default_flash_weightage_for_accuracy=0.1
        self.default_hdr_weightage_for_accuracy=0.2
       
        
        self.default_depth_sensor_weightage=0.1
        self.default_battery_consumption_weightage=0.1
        self.default_security_weightage=0.1

        #megapixels, flash and aperture will contribute to accuracy
        #ois, autofocus, hdr and flash will contribute to noise
        #battery_consumption to battery consumption
...
with open('C:/Users/karan/Desktop/Capstone Project Code and All/capstone-project/camera_dataset.csv','r') as f:
    w=csv.reader(f,delimiter=',')
    next(w)

    for r in w:
        # print(r)
        camera_sensor=camera(float(r[1]),int(r[2]),int(r[3]),int(r[4]),int(r[5]),int(r[6]),float(r[7]),int(r[8]),int(r[9]))
        if len(modes_to_apply_set)!=0:
            weight_for_each=max_weight/len(modes_to_apply)

            modes_to_apply_set_camera=set([])

            # for element in modes_to_apply_set:
            #     if element=="accuracy":
            #         modes_to_apply_set_camera.add("megapixels")

            #         modes_to_apply_set_camera.add("flash")

            #         modes_to_apply_set_camera.add("aperture")
                
            #     if element=="security":
            #         modes_to_apply_set_camera.add("security")

            #     if element=="battery_consumption":
            #         modes_to_apply_set_camera.add("battery_consumption")
                
            #     if element=="noise":
            #         modes_to_apply_set_camera.add("ois")
            #         modes_to_apply_set_camera.add("autofocus")
            #         modes_to_apply_set_camera.add("hdr")
            #         modes_to_apply_set_camera.add("flash")

                


            temp_set_all=set(["accuracy","battery_consumption","security","depth_sensor"])
            ...
            for n in remaining_set:
                if n=="security":
                    ...
                    camera_sensor.default_depth_sensor_weightage=other_weights
        ...
        megapixel_score=((((scale_max-scale_min) * (camera_sensor.megapixels-min_megapixels))/(max_megapixels-min_megapixels)) + scale_min)*camera_sensor.default_megapixels_weightage_for_accuracy

        accuracy_score=(megapixel_score  + camera_sensor.aperture*camera_sensor.default_aperture_weightage_for_accuracy + camera_sensor.flash * camera_sensor.default_flash_weightage_for_accuracy + camera_sensor.hdr*camera_sensor.default_hdr_weightage_for_accuracy + camera_sensor.ois*camera_sensor.default_ois_weightage_for_accuracy + camera_sensor.autofocus*camera_sensor.default_autofocus_weightage_for_accuracy) * camera_sensor.default_accuracy_weightage

        security_score=camera_sensor.security * camera_sensor.default_security_weightage

        battery_score=camera_sensor.battery_consumption*camera_sensor.default_battery_consumption_weightage
        depth_sensor_score=camera_sensor.depth_sensor * camera_sensor.default_depth_sensor_weightage
        ...
        camera_sensor.score=accuracy_score+security_score+battery_score+depth_sensor_score
